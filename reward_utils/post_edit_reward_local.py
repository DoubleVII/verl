...
async def group_post_edit_score_reward_fn(
    ...
    """GenRM reward function that calls a local colocated vLLM server via reward_router_address."""
    ...
    # call local vLLM server via reward_router_address
    assert reward_router_address is not None, "reward_router_address required for local GenRM"
    client = openai.AsyncOpenAI(base_url=f"http://{reward_router_address}/v1", api_key="EMPTY", timeout=rm_timeout)
    ...
    for attempt in range(rm_retry):
        try:
            ...
            print(f"[local_genrm] API call failed (attempt {attempt + 1}/{rm_retry}): {e}")
            continue

        scores = group_extract_scores(output_text, prompt_format, num_candidates)
        if scores is not None:
            break
        print(f"[local_genrm] Score extraction failed (attempt {attempt + 1}/{rm_retry})")
...
def batch_group_post_edit_score_reward_fn(
    ...
    if reward_router_address is None:
        raise ValueError("reward_router_address must be provided.")
    async def _run_all():
        tasks = [
    ...
    for result in results:
        if isinstance(result, Exception):
            print(f"[local_genrm] Item failed: {result}")
